...
public class Dog implements Pet {
     ...
     * 1) из класса А скопировать логику себе (это плохо, т.к. поддерживать несколько копий одного
     * и того же кода проблематично)
     ...
     * создайте метод private String getName(), который будет использовать логику двух классов -
     * Dog и SuperDog.
     * Пусть этот метод возвращает имя собаки (класс Dog), которое с обеих сторон выделено getSuperQuotes
     * (класс SuperDog)
     * Пример, " *** Барбос *** "
     * Логика метода say:
     * Если i < 1, то используя метод getName вывести на экран, что собака спит.
     * Пример, " *** Шарик ***  спит."'
    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable {
            ...
            @Override
            public String say() {
                return (i < 1) ? getSuperQuotes() + name + getSuperQuotes() + " спит." : getSuperQuotes() +
      name + getSuperQuotes() + " лает г" + String.format("%" + i +
                        "s","а").replaceAll("\\s","а") + "в! " + formatter.format(Calendar.getInstance().getTime());
                //return null;
            }
        }
       return new DogPet();
