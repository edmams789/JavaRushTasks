...
5. Метод containsBones в классе Finger должен возвращать строку "Yes", если метод containsBones
в классе BodyPart возвращает "Yes" и флаг isArtificial равен false, если приведенное условие
не выполняется - вернуть "No".
6. Метод toString в классе BodyPart должен возвращать строку формата "name(имя части тела)
содержит кости", если метод containsBones возвращает "Yes" для этой части тела, а если "No",
...
//Сразу понял что хотят, не ищите логику, это учебная задача, не имеющая связи с реальностью, а любое совпадение случайно.
//return containsBones().equals("Yes") ? name + " содержит кости" : name + " не содержит кости";
    public static class Finger extends BodyPart {
        private boolean isArtificial; //is Artificial - Искусственный
...
//"метод containsBones в классе BodyPart возвращает "Yes" и флаг isArtificial равен false" - читайте как
// "метод containsBones в классе BodyPart возвращает "Yes" и переменная isArtificial равна false"
...
//    {
//        List<BodyPart> bodyParts = new ArrayList<BodyPart>(5);
//// создаем список 5 элементов типа BodyPart
//        bodyParts.add(new BodyPart("Рука"));
//// создаем новый обьект Рука добавляем в список
//        bodyParts.add(new BodyPart("Нога"));
// // создаем нновый обьект Нога добавляем в список
//        bodyParts.add(new BodyPart("Голова"));
// // создаем новый обьект Голова добавляем в список
//        bodyParts.add(new BodyPart("Тело"));
//// создаем новый обьект Тело добавляем в список
//        System.out.println(bodyParts.toString());
// // выводим на экран список bodyParts обьектов BodyPart
//    }