...
Измени реализацию метода getExpectedMap, он должен вернуть объект такого класса, для которого будет
противоположное поведение при добавлении ссылки null.
...
/*
****** проверьте ключ "s", нет ли его на карте
карта НЕ содержит значения для ключа = s
карта НЕ содержит значения для ключа = s

****** проверьте ключ "s", находится ли он на карте
карта содержит значение для ключа = s
карта содержит значение для ключа = s

****** неоднозначное поведение для NULL
карта содержит значение для ключа = ноль
карта НЕ содержит значения для ключа = ноль
 */