...
/*
Можно не помечать поля модификатором public и не добавлять геттеры и сеттеры, а просто добавить аннотацию
@JsonAutoDetect(fieldVisibility = JsonAutoDetect.Visibility.ANY) перед классом.

При том работает корректно, если добавить аннотацию только к родительскому классу, но валидатор не пускает,
поэтому нужно добавлять всем. Аннотацию взял здесь здесь
 */